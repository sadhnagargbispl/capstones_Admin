...
public partial class AddKit : System.Web.UI.Page
{
    string GroupIdQS;
    ...
    private void Fill_SeriesStart()
    {
        try
        {
            string Sql = objDal.IsoStart + " Select Case When Max(KitId) Is Null Then '100001' Else (Max(KitId)+1)* 100000 +1 END as SrStart FROM " + objDal.DBName + "..M_KitMaster " + objDal.IsoEnd;
    ...
    private void BindData()
    {
        ...
        try
        {
            string sql = objDal.IsoStart + "Select * From " + objDal.DBName + "..m_KitMaster  Where KitId='" + KitIdQS + "' AND " + objDal.activeCondition + objDal.IsoEnd;
            Dt = new DataTable();
            Dt = SqlHelper.ExecuteDataset(constr1, CommandType.Text, sql).Tables[0];
            if (Dt.Rows.Count > 0)
            {
    ...
    protected void BtnSave_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            if (!string.IsNullOrEmpty(Request["KitId"]))
            {
                Str = "Insert into TempKitMaster([KId],[KitId],[KitName],[JoinAmount],[KitAmount],[KitUnit],[SerialStart],[RefIncome],[PoolIncome],[SpillIncome],[BinaryIncome],[BV],[PV],[RP],[Capping],[Remarks],";
                ...
                Str += "[JoinColor],[AllowTopUp],[Statement],[IsBill],[SP],[OnWebSite],[TopUpSeq],GetDate(),'" + Session["UserID"] + "' from M_Kitmaster as a where a.KitId='" + txtKitId.Text + "';";

                Sql = Str + ";Update M_KitMaster set KitId='" + txtKitId.Text + "',KitName='" + ClearInject(txtkitName.Text) + "',JoinAmount='" + txtJoinAmt.Text + "',";
            ...
            else
            {
                ...
                Sql = " insert into M_KitMaster (KitId,KitName,JoinAmount,KitAmount,KitUnit,SerialStart," +
                ...
                Sql += "'" + Convert.ToDouble(TxtTopUp.Text) + "' From " + objDal.tblKitMaster;
            }
            ...
            if (updateEffect > 0)
            {
                ...
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Successfully Updated.!');location.replace('KitMaster.aspx');", true);

                }
                else if (updateEffect > 0)
                {
                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Save Successfully.!');location.replace('KitMaster.aspx');", true);
                }
                else
                {
    ...
    protected void btnCancel_Click(object sender, EventArgs e)
    {
        try
        {
            Response.Redirect("KitMaster.aspx", false);
        }
        catch (Exception Ex)
        {
