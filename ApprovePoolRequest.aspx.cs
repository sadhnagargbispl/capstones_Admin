...
public partial class ApprovePoolRequest : System.Web.UI.Page
{
    string scrname;
    DAL objDAL = new DAL();
    ...
    public void BindData(string Condition = "")
    {
        try
        {
            ...
            string sql = objDAL.IsoStart + "exec Sp_PoolGetWalletApproveReport '" + idno + "','" + startDate + "','" + endDate + "',";
    ...
    private void AprvAction(string AprvType, string ApprvStatus)
    {
        try
        {
                ...
                if (Chk.Checked == true && Chk.Enabled == true)
                {
                    if (AprvType == "Y")
                    {
                        ...
                        sql = sql + "INSERT INTO RejtVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID,Status,Rn) SELECT ISNULL(Max(VoucherNo)+1,1001),'" + DateTime.Now.ToString("dd-MMM-yyyy") + "'," +
                            "'0','" + lbl.Text + "'," + LblAmount.Text + ",'Amount Added by Pool Payment Req.No.:" + lblReqno.Text + ".','Req/" + lblReqno.Text + "','P','C',Convert(Varchar,Getdate(),112),'" + Session["CurrentSessn"] + "','A',1 FROM TrnVoucher;";
                    }
                    else
                    {
                        Remark = " Reject Payment Request On ReqNo:" + lblReqno.Text + " for Idno:" + lblIdno.Text + "";
                    }
                    sql = sql + ";Update PoolWalletReq SET  IsApprove = '" + AprvType + "',ApproveDate=GEtdate(),ApproveBy='" + Convert.ToString(Session["UserID"]) + "',ApproveRemark='" + TxtARemark.Text + "' where FormNo='" + lbl.Text + "' And ReqNo='" + lblReqno.Text + "'";
            ...
            if (a != 0 && Cnt > 0)
            {
                ...
                TxtARemark.Text = "";
                BindData();
                Response.Redirect("ApprovePoolRequest.aspx");
            }
            else
            {
