...
public partial class FranchiseActivate : System.Web.UI.Page
{
    DAL Objdal = new DAL();
    string Sql = "";
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                Session["PageName"] = "Member / Franchise Investment ";
                if (!IsPostBack)
                {
    ...
    protected void BtnUpgrade_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            {
                string scrname;
                string Remark = " Franchise of Idno:" + TxtIDNo.Text + "";
                ...
                try
                {
                    ...
                    else
                    {
                        ...
                        if (Convert.ToInt32(txtAmount.Text) >= 10)
                        {
                            ...
                            {
                                if (amount >= 10 && amount <= 50000000000)
                                {
                                    Sql = "Exec Sp_packageActivateid '" + TxtIDNo.Text.Trim() + "',31," + amount + ";";
                                }
                                else
                                {
                                    // Out of range
                            ...
                            if (Objdal.SaveData(Sql) != 0)
                            {
                                ...
                                int updateEffect2 = Convert.ToInt32(SqlHelper.ExecuteNonQuery(constr, CommandType.Text, Sql));
                                    ...
                                    ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Franchise Investment Successfully.!');location.replace('FranchiseActivate.aspx');", true);
                               
                                lblError.Text = "Franchise Investment successfully.";
                                TxtIDNo.Text = "";
                                LblMemName.Text = "";
                                lblError.Text = "";
                                BtnUpgrade.Enabled = false;
            ...
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Something Want Wrong');location.replace('FranchiseActivate.aspx');", true);
            }
        }
        catch (Exception ex)
        {
