...
using System.Xml;
public partial class SalaryWithdraw : System.Web.UI.Page
{
    ...
    private void FillDetail()
    {
        try
        {
            ...
            string sql = objDAL.IsoStart + "exec Sp_SalaryWithReport '" + idno + "','" + startDate + "','" + endDate + "','" + RbtStatus.SelectedValue + "','N'" + objDAL.IsoEnd;
    ...
    protected void btnExport_Click(object sender, EventArgs e)
    {
        try
        {
            ...
            string sql = objDAL.IsoStart + "exec Sp_SalaryWithReport '" + idno + "','" + startDate + "','" + endDate + "','" + RbtStatus.SelectedValue + "','Y'" + objDAL.IsoEnd;
            dtTemp = SqlHelper.ExecuteDataset(constr1, CommandType.Text, sql).Tables[0];
            dg.DataSource = dtTemp;
            dg.DataBind();
            ExportToExcel("SalaryWithdrawalReport.xls", dg);
        }
        catch (Exception Ex)
        {
            ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('" + Ex.Message + "');", true);
        ...
        try
        {
            ...
            foreach (GridViewRow Gvr in GvData.Rows)
            {
                ...
                if (Chk.Checked)
                {
                    ...
                    Remark = "Salary Withdrawal Approved Of Idno " + TxtIDNo.Text + " For WeekNo:" + LnblWeek.Text + " By " + Session["UserName"];
                    string Sql = "";
                    Sql = "Update Salarywithdrawls Set Status = 'A',IssueDate = GETDATE(),Remark = '" + txtRemark.Text + "',UserId = '" + Session["UserID"] + "',UserName = '" + Session["UserName"] + "' Where ReqID = " + LblId.Text + ";" +
                          "Update TrnVoucher Set Userid='" + Session["UserId"] + "' Where DrTo='" + FormNo + "' And vtype='W' And VoucherDate='" + DateOn + "' And Narration Like 'Salary Debited Againest Bank Withdrawal on " + DateOn + " with req. no." + Id + "';" +
                          " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" +
                          "('" + Session["UserID"] + "','" + Session["UserName"] + "','Salary WithDrawals','Salary Withdrawls Approve','" + Remark + "',Getdate(),'" + FormNo + "')";
                    ...
                    if (updateeffect != 0)
                    {
                        ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('" + cnt1 + "Salary Withdrawal Approved Successfully.');", true);
                    }
                    else
                    {
        ...
        try
        {
            ...
            foreach (GridViewRow Gvr in GvData.Rows)
            {
                ...
                if (Chk.Checked)
                {
                    ...
                    string Remark = "Request Rejected: Salary Debited Against Bank Withdrawal with req. no. " + Id;
                    string RemarkBC = "Request Rejected: Salary Debited Against Bank Service Fees with req. no. " + Id;
                    ...
                    Str_TrnFun.Append("UPDATE Salarywithdrawls SET ");
                    ...
                    Str_TrnFun.Append(Convert.ToDecimal(lblNetAmount) + ",'" + Remark + "','" + refNo + "','W','C',CONVERT(VARCHAR,GETDATE(),112),");
                    Str_TrnFun.Append(sessQuery + " FROM TrnVoucher;");
                    // 3. Append Second INSERT (Admin Charge reversal)
                    Str_TrnFun.Append("INSERT INTO TrnVoucher(VoucherNo,VoucherDate,DrTo,CrTo,Amount,Narration,RefNo,AcType,VTYpe,SessID,WSessID) ");
                    Str_TrnFun.Append("SELECT ISNULL(MAX(VoucherNo)+1,1001),'" + voucherDate + "','0','" + FormNo + "',");
                    Str_TrnFun.Append(Convert.ToDecimal(lblAdminCharge) + ",'" + RemarkBC + "','" + refNo + "','W','C',CONVERT(VARCHAR,GETDATE(),112),");
                    Str_TrnFun.Append(sessQuery + " FROM TrnVoucher;");
            ...
            if (updateeffect != 0)
            {
                msg = cnt.ToString() + " Salary Withdrawal Rejected Successfully.!";
                FillDetail();
            }
            else
            {
        ...
        try
        {
            ...
            foreach (GridViewRow Gvr in GvData.Rows)
            {
                ...
                if (Chk.Checked)
                {
                    ...
                    string FormNo = ((Label)Gvr.FindControl("LblFormNo")).Text;
                    string DateOn = ((Label)Gvr.FindControl("LblDate")).Text;
                    string Remark = "Salary Withdrawal Approved Of Idno " + TxtIDNo.Text + " For WeekNo:" + LnblWeek.Text + " By " + Session["UserName"];
                    sql = "Update FundWithdrawls Set Status='" + ApprvStatus + "',IssueDate=GETDATE(),UserId='" + (Session["UserID"]) + "',UserName='" + Session["UserName"] + "' Where ReqID=" + LblId.Text + ";" +
                          " insert into UserHistory(UserId,UserName,PageName,Activity,ModifiedFlds,RecTimeStamp,MemberId)Values" +
                          "('" + (Session["UserID"]) + "','" + Session["UserName"] + "','Salary WithDrawals','Salary Withdrawls Virtual Approve','" + Remark + "',Getdate(),'" + FormNo + "')";
                    cnt1++;
                    updateeffect = Convert.ToInt32(SqlHelper.ExecuteNonQuery(constr, CommandType.Text, sql));
                    cnt++;
                }
            ...
            if (updateeffect != 0)
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('" + cnt1 + "Salary Withdrawal Virtual Approved Successfully.!');", true);
            }
            else
            {
                ScriptManager.RegisterStartupScript(this, this.GetType(), "Key", "alert('Server Timeout, Try After Some Time.!');", true);
